...
extern void printPyObj(PyObject *obj); 
extern void printPlgTerm(prolog_term term2);
...
int find_prolog_term_type(CTXTdeclc prolog_term term) {
  if(is_float(term))
    ...
    //      return PPYLIST;
    else if(strcmp(p2c_functor(term),"pyIter") == 0)
      return PYITER;
...
int convert_pyObj_prObj(CTXTdeclc PyObject *pyObj, prolog_term *prTerm) {
  ...
  //  else if(flag == 1 && PyList_Check(pyObj)) {
	...
	pkey = p2p_arg(head, 1);
	pval = p2p_arg(head, 2);
	convert_pyObj_prObj(CTXTc  key, &pkey);	
	convert_pyObj_prObj(CTXTc  value, &pval);	
	tail = p2p_arg(tail,2);
      }
      PyDict_Next(pyObj, &pos, &key, &value);
      c2p_functor(":",2,tail);
    ...
    *pyObj = pList;
	...
	PyTuple_SET_ITEM(tup,(i-1),arg);
      ...
      *pyObj = tup;
      ...
      if (!is_list(list)) {
	xsb_abort("++Error[janus]: py_set/1 contains an improper argument. "
	...
	list = p2p_cdr(list);
      ...
      *pyObj = pyset;
      ...
      pydict = PyDict_New();
      if (p2c_arity(prTerm) == 0) {
	*pyObj = pydict;
	return TRUE;
      }
      comma_list = p2p_arg(prTerm, 1);
      while (strcmp(p2c_functor(comma_list),",") == 0 ) {
